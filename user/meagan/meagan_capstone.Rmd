...
```{r}
# Define a function to crop the raster for a given ecoregion
crop_ecoregion <- function(raster, ecoregion) {
  cropped_raster <- crop(raster, ecoregion)
  return(cropped_raster)
}

# Create a list to store cropped raster objects
cropped_rasters <- list()

# Loop through the ecoregions
for (i in 1:nrow(eco_regions)) {
  # Extract the current ecoregion
  current_ecoregion <- eco_regions[i,]
  
  # Crop the raster to the current ecoregion using the function
  cropped_raster <- crop_ecoregion(tslf_raster_masked, current_ecoregion)
