...
gde_polygons_raster <- gde_data %>%
  st_transform(crs_ca) %>%
  filter(!grepl("Permanently Flooded", WETLAND_NA)) %>%
  mutate(gde = 1)
...
write.csv(gdepoly_fire_count,file="user/jillian/polygon_values/fire_count.csv",row.names=F)
beep(sound = 3)

# Find most recent TSLF in each GDE polygon
gdepoly_tslf <- exactextractr::exact_extract(tslf_final, gde_polygons_shiny, fun = "min")
# write.csv(gdepoly_tslf,file="user/jillian/polygon_values/tslf.csv",row.names=F)
beep(sound = 3)
...
write.csv(gdepoly_fire_threat, file = "user/jillian/polygon_values/fire_threat.csv", row.names = F)
beep(sound = 3)
...
write.csv(gdepoly_burn_severity, file = "user/jillian/polygon_values/burn_severity.csv", row.names = F)
beep(sound = 3)
...
for(id in ecoregion_codes){
  gdes_in_ecoregion <- gde_eco %>%
    filter(us_l3code == id) %>%
    select(POLYGON_ID, WETLAND_NA, VEGETATION, DOMINANT_S, DOMINANT_C, 
           max_fire_count, min_tslf, avg_fire_threat, mode_fire_severity, 
           avg_pct_burned, area, us_l3code, region, geometry)
  
  output_file <- paste0("raster_output/gde_summary_polygons/gde_summary_", id, ".shp")
...
# writeRaster(burn_severity_final, "raster_output/burn_severity_final.tif")
