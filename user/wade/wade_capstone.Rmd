...
eco_regions <- read_sf(here::here('user', 'wade', 'data', 'ca_eco_l3', 'ca_eco_l3.shp')) %>% 
  janitor::clean_names()  %>% 
  st_transform("EPSG: 3309") %>% 
  rename(region = us_l3name)

# Read in CA county boundaries
ca_counties <- read_sf(here::here('user', 'wade', 'data', 'CA_Counties', 'CA_Counties_TIGER2016.shp')) %>% 
  janitor::clean_names() %>% 
