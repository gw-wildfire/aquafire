...
fire_threat_mtns <- crop(fire_threat, mtns)
fire_threat_mtns <- mask(fire_threat_mtns, mtns)
```
...
fire_threat_coast <- crop(fire_threat, socal_baja_coast)
fire_threat_coast <- mask(fire_threat_coast, socal_baja_coast)
```

# Threat of 4 or threat of 5
```{r}
# mtns - Reassign values for medium and high count
fire_threat_mtns_med <- terra::ifel(fire_threat_mtns == 4, 4, NA) # moderate threat
fire_threat_mtns_high <- terra::ifel(fire_threat_mtns == 5, 5, NA) # high threat

# coast - Reassign values for medium and high count
fire_threat_coast_med <- terra::ifel(fire_threat_coast == 4, 4, NA) # moderate threat
fire_threat_coast_high <- terra::ifel(fire_threat_coast == 5, 5, NA) # high threat
```
...
```{r}
# coast
tm_shape(fire_threat_coast_med) +
  tm_basemap(leaflet::providers$Esri.WorldTerrain) +
  tm_raster(palette = c("#2A9D8F"),
            legend.show = FALSE,
            alpha = 0.4) +

  tm_shape(fire_threat_coast_high) +
  tm_basemap(leaflet::providers$Esri.WorldTerrain) +
  tm_raster(palette = c("#BC4749"),
            title = "Fire count",
            legend.show = FALSE) +
